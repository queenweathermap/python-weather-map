...
def plot_850hpa_temp_wind_700hpa_w(ax, ds, model="GSM", prop=None, skip=5):
    ...
    if model == "MSM":
        # MSMデータ：多次元から高度指定で抽出
        try:
            temp_850 = ds['t'].sel(isobaricInhPa=850).squeeze()  # [K]
            u_850    = ds['u'].sel(isobaricInhPa=850).squeeze()
            v_850    = ds['v'].sel(isobaricInhPa=850).squeeze()
            w_700    = ds['w'].sel(isobaricInhPa=700).squeeze()
        except Exception as e:
            print(f"Error extracting MSM levels: {e}")
            raise ValueError("Required 850/700hPa variables missing (MSM).")
    else:
        # GSMデータ：従来どおり
        ...
        if temp_850 is None or u_850 is None or v_850 is None or w_700 is None:
            raise ValueError("Required 850/700hPa variables missing (GSM).")
