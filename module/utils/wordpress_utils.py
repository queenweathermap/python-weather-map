...
#   ・画像を /media にアップロード → featured_media に設定して /posts を publish
#
...
def create_post(title: str, content_html: str, featured_media: int, *, status: str = "publish") -> dict:
    """
...
def post_climate_article(
    ...
    *,
    ...
    images: [(png_bytes, filename), ...]  本文にはこの順で並ぶ。
    featured_index: このindexの画像をfeatured_media（アイキャッチ）にする。
    ...
    try:
        ...
        featured_id = uploaded[featured_index]["id"]
        post = create_post(title, content_html, featured_media=featured_id, status="publish")
        url = post.get("link", "")
