...
# Playwrightでスクリーンショットする。1日8回実行し、地点ごとのraw画像を
...
# 表示される時間軸はサイト側で約7〜8.5時間分のローリングウィンドウ(うち
# 直近1〜2時間程度は未観測で空白)になっているため、3時間おきに実行すれば
# 十分な重なりを持って1日をカバーできる。GitHub Actionsのscheduleは
# best-effortで実行時刻が数十分〜数時間ずれることがあり、6時間おき(1日4回)
...
# から算出する。撮影間隔は通常3時間おきだが、GitHub Actionsのschedule遅延や
# フォールバック使用時など必ずしも3時間おきとは限らないため、ファイル名の
# 撮影時刻から実際の差分(時間)を計算して使う(build_daily_station_grid内)。
#
# 旧値82.5は実際のグリッド間隔より約8%小さく、実測し直すと89.75だった
# (2026-09-11の配信画像で複数地点・複数箇所の目盛り間隔を計測して確認)。
# 82.5のままだと、実際の撮影間隔が長い(=貼り合わせ位置のずれが大きい)
# 継ぎ目ほど貼り付け位置が本来より手前寄りになり、隙間が目立っていた
# (ユーザー報告「13時と14時の間だけ間隔が狭い」の原因。実際には全ての
...
def build_daily_station_grid(dt_jst: datetime, *, cols: int = 5) -> Tuple[bytes, int]:
    ...
    各画像はサイト側のローリングウィンドウ（約7〜8.5時間分）をそのまま切り出した
    もので、実行間隔(3時間)より長いため隣り合うコマは十分重なる。真に継ぎ目のない
...
def main() -> int:
    """1日8回実行し、地点ごとのraw画像をR2へ個別保存するだけ。
