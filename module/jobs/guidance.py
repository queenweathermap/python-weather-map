...
def screenshot_wcn_amedas_pages() -> List[Tuple[str, bytes]]:
    ...
    def _submit_form(page, select_name: str, value: str):
        """form_area の select を変更して決定ボタンを押す。"""
        ff = page.frame(name="form_area")
        if not ff:
            raise RuntimeError("form_area not found")
    ...
    def _select_by_idx(page, idx: int):
        """form_area の最初の select をインデックスで変更して決定。"""
        ff = page.frame(name="form_area")
        if not ff:
            raise RuntimeError("form_area not found")
    ...
    def _debug_form(page) -> List[str]:
        """form_area の select option テキストをデバッグ出力。"""
        ff = page.frame(name="form_area")
        if not ff:
            return []
        try:
    ...
    with sync_playwright() as pw:
        ...
        # ---- 1. allamedas.html ----------------------------------------
        ALLAMEDAS_ITEMS = [
            # (select_name, option_value, fname, label)
            ("element", "rain3h",  "amedas_rain3h.png",  "アメダス 3時間降水量"),
            ("element", "tmax",    "amedas_tmax.png",    "アメダス 最高気温"),
            ("element", "tmin",    "amedas_tmin.png",    "アメダス 最低気温"),
            ("element", "wmax",    "amedas_wmax.png",    "アメダス 最大風速"),
            ("element", "humin",   "amedas_humin.png",   "アメダス 最小湿度"),
            ("element", "snow",    "amedas_snow.png",    "アメダス 積雪深"),
        ]
        for select_name, value, fname, lbl in ALLAMEDAS_ITEMS:
            try:
                ...
                try:
                    ...
                    # select_name が不明なら最初の select で試す
                    ff = page.frame(name="form_area")
                    if ff:
                        sel = ff.locator("select").first
                        try:
                            ...
                            for opt in sel.locator("option").all():
                                txt = opt.inner_text()
                                if value in txt or lbl.split()[-1] in txt:
        ...
        # ---- 2. ranking.html ------------------------------------------
        # ランキング3グループを1枚ずつスクリーンショット
        # ranking_2.cgi は全カテゴリを縦に並べる想定。
        # form_area の select でカテゴリを切り替え、決定後に data_area を全高撮影する。
        RANKING_GROUPS = [
            # (fname, label, select_values)
            # select_values: フォームで選択する値のリスト（複数ある場合は順に試す）
            (
                "ranking_temp.png",
                "ランキング 気温（最高↑ / 最低↓ / 低最高↓ / 高最低↑）",
                ["temp"],          # フォームに温度系カテゴリが "temp" 等の値の場合
            ),
            (
                "ranking_rain.png",
        ...
        # まずページを開いてフォームの option 一覧を確認
        try:
            page.goto(WCN_RANKING_URL, wait_until="networkidle", timeout=60_000)
            _wait(page)
            rank_opts_raw = _debug_form(page)
            print(f"[DEBUG] ranking opts: {rank_opts_raw}")
        except Exception as e:
            print(f"[WARN] ranking.html ロード失敗: {e}")
            rank_opts_raw = []

        # オプションが3つ以上あれば index で取得
        for idx, (fname, lbl, _) in enumerate(RANKING_GROUPS):
            try:
                page.goto(WCN_RANKING_URL, wait_until="networkidle", timeout=60_000)
                _wait(page)

                if len(rank_opts_raw) >= 3:
                    # index でオプション選択
                    _select_by_idx(page, idx)
                else:
                    # オプション1つのみ（全カテゴリ表示）—そのままスクロール撮影
                    print(f"[INFO] ranking options < 3, full-page screenshot for {fname}")
...
def post_wcn_amedas_to_discord(
    webhook_url: str = "",
    images: Optional[List[Tuple[str, bytes]]] = None,
) -> None:
    """WCN アメダス画像を Discord の #amedas チャンネルへ投稿。"""
    url = webhook_url or DISCORD_AMEDAS_WEBHOOK_URL
    if not url:
    ...
    # グループごとに分けて投稿
    allamedas = [(f, b) for f, b in images if f.startswith("amedas_")]
    rankings  = [(f, b) for f, b in images if f.startswith("ranking_")]

    import urllib.request, urllib.parse, json as _json, email.generator, io, uuid
    ...
    def _post_multipart(files: List[Tuple[str, bytes]], content: str = "") -> None:
        ...
        for i, (fname, data) in enumerate(files):
            ...
            body_parts_bytes = "".join(body_parts).encode() + data + b"\r\n"
            body_parts = []
            if i == 0:
                raw_body = body_parts_bytes
            else:
                raw_body += body_parts_bytes  # type: ignore[operator]
        raw_body += f"--{boundary}--\r\n".encode()  # type: ignore[operator]

        # simpler: build with explicit bytes
        body = b""
        if content:
            body += (
                f'--{boundary}\r\n'
    ...
    if rankings:
        for i, item in enumerate(rankings):
            _post_multipart([item], content="" if i > 0 else "**アメダスランキング（WCN）**")
